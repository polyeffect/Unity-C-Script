...
public class camera_frustum_display : MonoBehaviour {
	...
	void OnDrawGizmos(){
		...
		m2[1, 1] *= GetComponent<Camera>().aspect;
		Gizmos.matrix = m * m2;
		Gizmos.DrawFrustum( transform.position, GetComponent<Camera>().fieldOfView, GetComponent<Camera>().farClipPlane, GetComponent<Camera>().nearClipPlane, GetComponent<Camera>().aspect );
	}
